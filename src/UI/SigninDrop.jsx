...
export default function SigninDrop() {
  let dispatch = useDispatch();
  const onDrop = useCallback((acceptedFiles) => {
    acceptedFiles.forEach((file) => {
      dispatch({ type: "addLogo/addFile", file });
    });
  }, []);
  const { getRootProps, getInputProps, isDragActive } = useDropzone({ onDrop });

  let files = useSelector((state) => state.signin.files);
  ...
  return (
    <div className="flex flex-col gap-2 h-40 mb-8">
      <div
        {...getRootProps()}
        ...
        {fileNames.map((fileName) => (
          ...
          >
            ...
            <button
              ...
              type="button"
              onClick={() => {
                dispatch({ type: "addLogo/deleteFile", fileName });
              }}
            >
