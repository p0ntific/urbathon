...
export default function ComplainDrop() {
  let dispatch = useDispatch();
  const onDrop = useCallback((acceptedFiles) => {
    acceptedFiles.forEach((file) => {
      dispatch({ type: "complain/addFile", file });
    });
  }, []);
  const { getRootProps, getInputProps, isDragActive } = useDropzone({ onDrop });

  let files = useSelector((state) => state.complain.files);
        ...
        {fileNames.map((fileName) => (
          ...
          >
            ...
            <button
              ...
              type="button"
              onClick={() => {
                dispatch({ type: "complain/deleteFile", fileName });
              }}
            >
